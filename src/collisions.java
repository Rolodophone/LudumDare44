class collisions {
     ...
     */
    static Collision findEntranceCheckForTunnelling(float ix, float iy, float fx, float fy, float w, float h, float bx, float by, float bw, float bh) {
        //--------SET UP VARS--------------
        //bl means base left
        float bl = bx;
        float br = bx + bw;
        ...
        if (contactX != null && contactX + w > bl && contactX < br) {
            return new collisions.Collision(contactX, contactY, "top");
        }
        ...
        if (contactX != null && contactX + w > bl && contactX < br) {
            return new collisions.Collision(contactX, contactY, "bottom");
        }
    ...
    static private Float computePoint(float ix, float fx, float cx, float iy, float fy, boolean distShouldBePositive) {
        ...
        if (xFrac >= 1 || xFrac <= 0)
            return null; // if xFrac > 1, that means contactRect is not between initialRect and finalRect, so there is no collision

        float yDist = (fy - iy) * xFrac; // times the distance fy to iy by the same fraction to get the y distance from iy to cy

        return iy + yDist; // Add that distance to iy to get cy
    }
    ...
    final static class Collision {
         ...
         */
        Collision(Float x, Float y, String s) {
            this.x = x;
            this.y = y;
            this.side = s;
        }
